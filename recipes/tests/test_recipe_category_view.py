...
class RecipeCategoryViewTest(RecipeTestBase):
    ...
    def test_recipe_category_view_return_if_no_recipes(self):
        response = self.client.get(reverse('recipe:category', kwargs={'name': 'Carnes'}))  # noqa: E501
        self.assertIn('No recipes found here!', response.content.decode('utf-8'))  # noqa: E501

    def test_recipe_category_template_loads_recipe(self):
        self.make_recipe(title="Ola teste")
        response = self.client.get(reverse('recipe:category', args=('Category', )))  # noqa: E501
